...
import svgPath from "../assets/1JMC.svg";
import jmc1_3DModel from "../assets/3d-models/jmc-1.glb";
import "../App.css";
...
function App() {
  ...
  const handleFloorNavigation = () => {
    navigate('/jmc-second-floor');  // Adjust this path to match your router configuration
  };
  ...
  // Add useEffect for event listeners
  useEffect(() => {
    const container = document.querySelector('.floor-plan-container');
    if (container) {
  ...
  // Add these functions near your other zoom handlers
  const handleHorizontalPan = (direction) => {
    const panAmount = 50; // Adjust this value to control pan sensitivity
    setPan(prev => ({
